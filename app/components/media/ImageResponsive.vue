...
function getImageSize(imageAsset: StoryblokAsset, imageRatio?: string | number) {
  ...
  // If no ratio provided, return original image dimensions
  if (!imageRatio) {
    return {
      width,
      height,
  ...
  const isRatioValid = validAspectRatio(imageRatio)
...
const mobileSize = computed(() => getImageSize(asset, ratio))
const imgInfo = computed(() => createImageInfo(asset, mobileSize.value, sizes))
const mobilePlaceholder = computed(() => createPlaceholder(asset, mobileSize.value))

// Desktop image size and info (if available)
const hasDesktopImage = computed(() => desktopAsset?.filename)
const desktopSize = computed(() => hasDesktopImage.value ? getImageSize(desktopAsset!, desktopRatio) : mobileSize.value)
...
const imgAttrs = computed(() => ({
  ...rest,
  width: mobileSize.value.width,
  height: mobileSize.value.height,
...
<template>
  ...
  >
    <picture
      ref="container"
      class="media-image__file size-full"
    ...
    <picture
      ...
      class="media-image__placeholder size-full"
      :class="{ 'flex items-center justify-center': cover }"
    >
